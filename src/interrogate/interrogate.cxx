...
// output_include_filename.set_text();
  ...
  // The C# backend consumes the C wrappers via P/Invoke, so we need exported
  // C wrapper functions with stable names, string conversion, and reference
  // count management.  The supplemental pass generates both the C wrappers
  // and any C# native helpers (destructors, collection stubs) in one file.
  // The .cs managed bindings are emitted separately by interrogate_csharp (pass 2).
  if (build_csharp) {
    // The C# backend consumes C wrappers via P/Invoke.  The supplemental pass
    // compiles only the _csharp_supplemental.cxx file (not _igate_base.cxx),
    // so that file must contain the complete set of C wrappers.
    build_c_wrappers = true;
    output_function_names = true;
    convert_strings = true;
    manage_reference_counts = true;
  ...
  // The .in database is written AFTER write_code() so that wrapper indices
  // from InterfaceMakerC (and others) are recorded before serialisation.
  // (The old "write before write_code" path for build_csharp was a bug when
  // build_c_wrappers was also true — the C wrappers hadn't been generated yet.)
