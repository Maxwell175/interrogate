...
// output_include_filename.set_text();
  ...
  // count management.  The C wrappers themselves are generated by the normal
  // C backend pass; the C# pass only adds supplemental native helpers
  // (destructors, collection stubs).  The .cs files are emitted separately
  // by interrogate_csharp (pass 2).
  if (build_csharp) {
    output_function_names = true;
    convert_strings = true;
    manage_reference_counts = true;
  ...
  // Write the database BEFORE write_code only for the legacy inline C# path:
  // that backend may merge external .in databases during write_code, which
  // would contaminate the output if serialized afterward.  Pure C / Python
  // passes still need write_code to run first so generated wrapper indices are
  // recorded into the .in file.
  if (build_csharp && !output_data_filename.empty()) {
